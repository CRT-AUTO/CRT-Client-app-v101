  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const loadPromises = {
          connections: getSocialConnections().catch(err => {
            console.error('Error loading social connections:', err);
            return [];
          }),
          mappings: getVoiceflowMappings().catch(err => {
            console.error('Error loading Voiceflow mappings:', err);
            return [];
          }),
          stats: getDashboardStats(user.id).catch(err => {
          ...
          analytics: getMessageAnalytics(user.id).catch(err => {
            console.error('Error loading message analytics:', err);
            return [];
          }),
          conversations: getRecentConversations(user.id).catch(err => {
            console.error('Error loading recent conversations:', err);
            return [];
          }),
          hourlyData: getMessageVolumeByHour(user.id).catch(err => {
        ...
        setSocialConnections(results[0] || []);
        
        const mappings = results[1];
        if (mappings && mappings.length > 0) {
          setVoiceflowMapping(mappings[0]);
        }
        
        setStats(results[2]);
        setMessageAnalytics(results[3] || []);
        setRecentConversations(results[4] || []);
        setHourlyVolume(results[5] || []);
