...
export async function initVoiceflowConfig(): Promise<boolean> {
  try {
    ...
    try {
      ...
      console.log('No API key found, continuing with default configuration');
    }
...
export async function processMessageWithVoiceflow(
  ...
  try {
    ...
    // If all retries fail, return null or throw
    return null;
  }
}
...
export async function handleIncomingMessage(
  ...
  try {
    ...
    if (getConversationError) throw getConversationError;
